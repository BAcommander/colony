...
Current scene (2026-09-25): Basalt Transmission. V2 still failed the user's visibility goal: "really struggling to see it". V3 changes the sky method to actual source-cloud detail advection with a smooth regional transition, rather than faint procedural modulation. Dust gusts travel480sourcepixels per20seconds (v2:190), are1.5x wider and1.6x thicker with greater density. Eight gusts remain behind rock masks; buildings, camera, moon and habitat effects stay fixed. Configuration/brief: creative/basalt-transmission/animation/scene-plan-v3.json and production-prompt-v3.md. Render and assemble with --version v3. Current exports use baseline-v3; older versions remain unchanged. See versioned validation reports for completed checks. User artistic review is pending; Ringfall remains the approved reference. Videos remain local; long deliveries exceed ordinary GitHub limits.
