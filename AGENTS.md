...
Current scene (2026-09-25): Basalt Transmission, source creative/concepts/09-basalt-transmission.png. The pasted production brief authorized execution. Implemented with reusable scripts/ambient_effects.py, config-driven scripts/render_basalt.py and scripts/assemble_basalt.py. Two mapped roof vents, terrain-occluded valley haze and two independent small-room light schedules; main room steady. Optional sky/mast effects omitted. Current delivery is creative/basalt-transmission/animation/baseline-v1b-20min-4k.mp4: twenty-second loop repeated sixty times, silent, upscaled4K. User artistic review pending; Ringfall remains the approved reference. See the scene README and validation reports for exact verification/storage status. The long delivery is about664MB, local-only and too large for ordinary GitHub file storage; do not claim it was pushed. Supporting assets/code/reports are tracked.
