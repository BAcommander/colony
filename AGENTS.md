...
Current scene (2026-09-25): Basalt Transmission v4 is a short balance-review pass. User found v3 middle-right smoke excessive while the far distance/top still looked static. Reduce near gust density/height and near haze; add separately masked far-valley/foothill haze and extend source-cloud advection into the upper sky. Protect the moon both in the output mask and by inpainting it out of the texture-only source to prevent duplicate moons. Configuration/brief: creative/basalt-transmission/animation/scene-plan-v4.json and production-prompt-v4.md. Renderer supports --version v4. Deliver20-second720p and4K review loops, with a v3/v4 comparison; do not rebuild another20-minute file until this distribution has been reviewed. Existing v3 long delivery remains intact. User artistic approval pending; Ringfall is the approved reference.
